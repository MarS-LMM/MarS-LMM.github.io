...
function drawMainChart1(container, chartContainer) {
    ...
    function splitData(rawData) {
        ...
        for (var i = 0; i < rawData.length; i++) {
            ...
            if (i <= 100) {
                values.push([data1[i].open, data1[i].close, data1[i].low, data1[i].high]);
    ...
    var chartIntervalId = setInterval(function() {
        ...
        const random = data1[current] ? current + n : current - 150 + n
        data0.values[current] = [data1[random].open, data1[random].close, data1[random].low, data1[random].high]
        const bids = generateOrderLevels('bid', 5, 62, false);
        data0.volume[current] = data0.volume[current] ? bids[0].volume + data0.volume[current] : bids[0].volume
        myChart.setOption({
            series: [
                {
                    data: data0.values,
...
function static_chart_demo2(contrainer) {
    ...
    function splitData(rawData) {
        ...
        for (var i = 0; i < 50; i++) {
            ...
            const bids = generateOrderLevels('bid', 5, 62, false);
            volume.push(bids[0].volume);
            TWAPData.push(Math.floor(Math.random() * 800));
            rollout.push(Math.floor(Math.random() * 500));
            value2.push(null)
...
function drawDemo2V1_1(contrainer, downloadButtonContainer) {
    console.log(1)
    const lightTheme = {
        chart: {
            layout: {
